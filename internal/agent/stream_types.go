...
func (s StreamMessage) MarshalJSON() ([]byte, error) {
	...
	if s.Type == "result" {
		...
		// ttft_ms: time to first token (always emitted per reference format)
		fields = append(fields, fmt.Sprintf(`"ttft_ms":%d`, s.TTFTMs))
		// ttft_stream_ms: time to first stream event (always emitted per reference format)
		fields = append(fields, fmt.Sprintf(`"ttft_stream_ms":%d`, s.TTFTStreamMs))
		// time_to_request_ms: pre-API processing time (always emitted per reference format)
