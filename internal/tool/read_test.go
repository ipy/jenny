...
func TestReadTool_ImageFile_NoVisionModel(t *testing.T) {
	tmpDir := t.TempDir()
	...
	tool := NewReadTool(PermissionUnrestricted, nil)
	tool.WithModelName("deepseek-v4-flash")
	result, err := tool.Execute(context.Background(), map[string]any{
		"file_path": imgFile,
	}, tmpDir)
	if err != nil {
	...
	if !result.IsError {
		t.Fatalf("expected error for non-vision model, got content: %s", result.Content)
	}
	if !strings.Contains(result.Content, "does not support image/vision input") {
		t.Errorf("expected 'does not support image/vision input' error, got: %s", result.Content)
	}
}

func TestReadTool_ImageFile_VisionModel(t *testing.T) {
	tmpDir := t.TempDir()
	...
	tool := NewReadTool(PermissionUnrestricted, nil)
	tool.WithModelName("claude-sonnet-4-20250514")
	result, err := tool.Execute(context.Background(), map[string]any{
		"file_path": imgFile,
	}, tmpDir)
	if err != nil {
	...
	if result.IsError {
		t.Fatalf("expected success for vision model, got error: %s", result.Content)
	}
	if !strings.HasPrefix(result.Content, "data:image/png;base64,") {
		t.Errorf("expected data URI, got: %.40s...", result.Content)
	}
...
func TestReadTool_ImageFile_EmptyModel(t *testing.T) {
	tmpDir := t.TempDir()
	...
	tool := NewReadTool(PermissionUnrestricted, nil)
	// No WithModelName call — model name is empty
	result, err := tool.Execute(context.Background(), map[string]any{
		"file_path": imgFile,
	}, tmpDir)
	if err != nil {
	...
	if result.IsError {
		t.Fatalf("expected success with empty model name, got error: %s", result.Content)
	}
	if !strings.HasPrefix(result.Content, "data:image/png;base64,") {
		t.Errorf("expected data URI, got: %.40s...", result.Content)
	}
