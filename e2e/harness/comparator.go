...
func Compare(tc *TestCase, actual *CapturedOutput, workDir string) CompareResult {
	...
	// FileSystem checks
	if len(tc.Expected.FileSystem) > 0 {
		diffs = append(diffs, compareFileSystem(tc.Expected.FileSystem, workDir, actual.Stdout)...)
	}
...
func compareFileSystem(fs []FileSystemExpectation, workDir string, stdout string) []DiffDetail {
	var diffs []DiffDetail
	for _, fse := range fs {
		if fse.Path != "" {
			diffs = append(diffs, checkFilePath(fse, workDir, stdout)...)
		} else if fse.Pattern != "" {
			diffs = append(diffs, checkFilePattern(fse, workDir, stdout)...)
		}
	}
	return diffs
}

func checkFilePath(fse FileSystemExpectation, workDir string, stdout string) []DiffDetail {
	var diffs []DiffDetail
	fullPath := filepath.Join(workDir, fse.Path)
	...
	if fse.MustNotExist {
		if exists {
			diffs = append(diffs, DiffDetail{
				Path:    "filesystem",
				Message: fmt.Sprintf("file %s expected not to exist, but it does", fse.Path),
			})
		}
		return diffs
	}
	...
	if !exists {
		diffs = append(diffs, DiffDetail{
			Path:    "filesystem",
			Message: fmt.Sprintf("file %s expected to exist, but not found", fse.Path),
		})
		return diffs
	}

	if fse.Content != "" && string(data) != fse.Content {
		diffs = append(diffs, DiffDetail{
			Path:     "filesystem",
			Expected: fse.Content,
			Actual:   string(data),
			Message:  fmt.Sprintf("file %s content mismatch", fse.Path),
		})
	}

	if fse.JSONL != nil {
		diffs = append(diffs, checkJSONL(fse.JSONL, fullPath, data, stdout)...)
	}

	return diffs
}

func checkFilePattern(fse FileSystemExpectation, workDir string, stdout string) []DiffDetail {
	var diffs []DiffDetail
	matches, err := filepath.Glob(filepath.Join(workDir, fse.Pattern))
	if err != nil {
		return append(diffs, DiffDetail{Path: "filesystem", Message: fmt.Sprintf("bad glob pattern %s", fse.Pattern)})
	}

	if fse.MustNotExist {
		if len(matches) > 0 {
			diffs = append(diffs, DiffDetail{Path: "filesystem", Message: fmt.Sprintf("pattern %s expected no matches, but found %d", fse.Pattern, len(matches))})
		}
		return diffs
	}

	if fse.ExpectedCount > 0 && len(matches) != fse.ExpectedCount {
		diffs = append(diffs, DiffDetail{
			Path:     "filesystem",
			Expected: fse.ExpectedCount,
			Actual:   len(matches),
			Message:  fmt.Sprintf("pattern %s match count mismatch", fse.Pattern),
		})
	}

	// For pattern matches, we apply the JSONL assertion to all matching files
	if fse.JSONL != nil {
		for _, match := range matches {
			data, _ := os.ReadFile(match)
			diffs = append(diffs, checkJSONL(fse.JSONL, match, data, stdout)...)
		}
	}
...
func checkJSONL(exp *JSONLExpectation, filePath string, data []byte, stdout string) []DiffDetail {
	var diffs []DiffDetail
	...
	for i, line := range lines {
		...
		if exp.AllLinesValidJSON && err != nil {
			diffs = append(diffs, DiffDetail{Path: fmt.Sprintf("%s:%d", filePath, i), Message: "invalid JSON line"})
			continue
		}
		...
		if err == nil {
			...
			if exp.AllLinesHaveUUID {
				if u, ok := obj["uuid"].(string); !ok || !IsValidUUID(u) {
					diffs = append(diffs, DiffDetail{Path: fmt.Sprintf("%s:%d", filePath, i), Message: "missing or invalid uuid field"})
				}
			}

			for _, reqField := range exp.RequiredFields {
				if _, ok := obj[reqField]; !ok {
					diffs = append(diffs, DiffDetail{Path: fmt.Sprintf("%s:%d", filePath, i), Message: fmt.Sprintf("missing required field %s", reqField)})
				}
			}
		}
	}

	if exp.MinCount > 0 && validCount < exp.MinCount {
		diffs = append(diffs, DiffDetail{Path: filePath, Expected: fmt.Sprintf(">= %d lines", exp.MinCount), Actual: validCount, Message: "not enough valid JSONL lines"})
	}
	if exp.MaxCount > 0 && validCount > exp.MaxCount {
		diffs = append(diffs, DiffDetail{Path: filePath, Expected: fmt.Sprintf("<= %d lines", exp.MaxCount), Actual: validCount, Message: "too many valid JSONL lines"})
	}

	for _, wantType := range exp.HasTypes {
		if !foundTypes[wantType] {
			diffs = append(diffs, DiffDetail{Path: filePath, Message: fmt.Sprintf("missing expected event type %s", wantType)})
		}
	}

	if exp.SessionIDMatchesStdout {
		// Extract session_id from stdout
		var sid string
		for _, sLine := range strings.Split(stdout, "\n") {
			var sObj map[string]any
			if json.Unmarshal([]byte(sLine), &sObj) == nil {
				if id, ok := sObj["session_id"].(string); ok && id != "" {
					sid = id
					break
				}
			}
		...
		if sid != "" && stem != sid {
			diffs = append(diffs, DiffDetail{Path: filePath, Expected: sid, Actual: stem, Message: "file stem does not match session_id from stdout"})
		}
	}
