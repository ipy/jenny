...
// runE2ESuiteWithDelay runs the test suite with a mock API delay for timing tests.
func runE2ESuiteWithDelay(t *testing.T, tests []*harness.TestCase, delayMs int) {
	t.Helper()
	cfg := defaultConfig()
	runner := harness.NewSuiteRunner(cfg, tests)
	// Set delay on the mock server if delayMs > 0
	if delayMs > 0 {
		runner.SetMockDelay("echo-hello", delayMs)
	}
	reporter := &harness.TextReporter{}
	results := runner.RunAll(reporter)
